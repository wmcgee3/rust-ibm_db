...
use ibm_db::{
    SQLAllocHandle, SQLExecDirect, SQLFreeHandle, SQLFreeStmt, SQLSetConnectAttr, SQLCHAR,
    SQLPOINTER, SQLSMALLINT, SQL_ATTR_AUTOCOMMIT, SQL_AUTOCOMMIT_ON, SQL_CLOSE, SQL_HANDLE_DBC,
    SQL_HANDLE_ENV, SQL_HANDLE_STMT, SQL_NO_DATA_FOUND, SQL_NTS, SQL_RESET_PARAMS, SQL_SUCCESS,
    SQL_UNBIND,
};
...
fn connect(mut dsn: String, mut uid: String, mut pwd: String) -> i32 {
    unsafe {
        ...
        let mut cliRC;
        let mut out = 0;
        cliRC = SQLAllocHandle(SQL_HANDLE_ENV as i16, 0, &mut out);
        if cliRC != SQL_SUCCESS as i16 {
            println!(
                "--ERROR while allocating the environment handle. Status: {}",
                cliRC
            );
            return 0;
        }

        cliRC = SQLAllocHandle(SQL_HANDLE_DBC as i16, out, &mut hdbc);
        if cliRC != SQL_SUCCESS as i16 {
        ...
        SQLSetConnectAttr(
            hdbc,
            SQL_ATTR_AUTOCOMMIT as i32,
            SQL_AUTOCOMMIT_ON as SQLPOINTER,
            SQL_NTS,
        );

        let mut hstmt = 0;

        cliRC = SQLAllocHandle(SQL_HANDLE_STMT as SQLSMALLINT, hdbc, &mut hstmt);
        if cliRC != SQL_SUCCESS as i16 {
